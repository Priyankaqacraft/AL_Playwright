...
describe('Author: ', () => {
  ...
  it('It Should verify that webinar Registration page is showing validation message for null values', async () => {
    ...
    try {
      ...
      const eml= await commonPage.getElementText(webinar.emailrequired);
  ...
  it('It should verify that webinar registration is showing error message when we try to do register with already existed email id', async () => {
    ...
    try {
      ...
      await commonPage.clearAndSendKeys(webinar.cmpname, 'abc');
      ...
      expect( emailmessage).toEqual("This email address already has a verified Acelab account associated with it. Please sign-in instead.");
