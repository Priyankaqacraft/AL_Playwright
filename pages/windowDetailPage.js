...
class WindowDetailPage extends CommonPage {
    ...
    /* async checkFileDownloaded(filename) {
        ...
        async checkFileDownloaded(filename) {
            const maxRetries = 5;
            const retryInterval = 500; // in milliseconds
            const freshnessDuration = 2 * 60 * 1000; // 2 minutes in milliseconds
            const currentDate = new Date().toISOString().slice(0, 10); // e.g., "2024-11-14"
        
            for (let i = 0; i < maxRetries; i++) {
                // Get list of files in the download path
                const files = fs.readdirSync(this.downloadPath);
                
                // Filter files by filename containing the provided name and the current date
                const matchedFile = files
                    ...
                    .sort((a, b) => b.lastWriteTime - a.lastWriteTime)[0]; // Get the most recent file
        
                // If a matching file is found, check its freshness
                if (matchedFile) {
                    const isFresh = Date.now() - matchedFile.lastWriteTime.getTime() < freshnessDuration;
                    fs.unlinkSync(matchedFile.fullPath); // Delete the file if needed
                    return isFresh; // Return true if the file is fresh
                }
